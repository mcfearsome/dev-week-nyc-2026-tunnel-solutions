    ...
    loop {
        match next_frame(&mut ws).await? {
            AgentFrame::Result { content, .. } => {
                let text = content.get(0).and_then(|c| c.get("text")).and_then(|t| t.as_str())
                    .context("request_file result had no text")?.to_string();
                let (magnet, peers) = parse_request_file_content(&text);
                let name = name_from_magnet(&magnet);
                return Ok(RetrievedFile { magnet, peers, name });
            }
            AgentFrame::Error { code, message, .. } =>
...
pub async fn run_get(link: &str, out_dir: &std::path::Path) -> Result<()> {
    ...
    let dl = crate::bittorrent::fetch(&rf.magnet, out_dir, crate::bittorrent::NetOpts {
        disable_dht: false, listen_port: None, enable_upnp: true, initial_peers: rf.peers,
    }).await?;
    loop {
        let p = dl.progress();
